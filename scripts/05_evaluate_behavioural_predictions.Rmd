...
The mean absolute prediction error is a linear error metric.
We could also choose a non-linear metric that penalises larger deviations more heavily.
A common choice would be RMSE (root mean squared error).
...
Use `emmeans` to estimate the standardised effect size for each contrast:
```{r}
emm_acc_gl <- emmeans(m_acc_pred_error_gl, "prediction_label", lmer.df = "asymptotic")
# Sigma calculation follows the example in the eff_size documentation (see also Westfall et al., 2014)
vc_acc_gl <- as.data.frame(VarCorr(m_acc_pred_error_gl)) 
sigma_acc_gl <- sqrt(sum(vc_acc_gl$vcov))
eff_size_acc_gl <- eff_size(emm_acc_gl, sigma = sigma_acc_gl, edf = Inf) # Choice of edf does not affect effect size, only the SE
eff_size_acc_gl
```
...
Use `emmeans` to estimate the standardised effect size for each contrast:
```{r}
emm_acc_ss <- emmeans(m_acc_pred_error_ss, "prediction_label", lmer.df = "asymptotic")
# Sigma calculation follows the example in the eff_size documentation (see also Westfall et al., 2014)
vc_acc_ss <- as.data.frame(VarCorr(m_acc_pred_error_ss)) 
sigma_acc_ss <- sqrt(sum(vc_acc_ss$vcov))
eff_size_acc_ss <- eff_size(emm_acc_ss, sigma = sigma_acc_ss, edf = Inf) # Choice of edf does not affect effect size, only the SE
eff_size_acc_ss
```
...
# Associated standardised effect size
eff_size_acc_gl_tidy <- broom::tidy(eff_size_acc_gl) |> as.data.table()
eff_size_acc_gl_tidy[contrast == "Default - Fact & Learner", estimate]
```
